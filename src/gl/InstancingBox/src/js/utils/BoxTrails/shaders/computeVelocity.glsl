...
void main() {
    ...
    float nScale = 0.3;
    vel.xyz += 20.0 * vec3(
      snoise( vec4( nScale * pos.xyz, 7.225 + seed + 0.4 * time ) ),
      snoise( vec4( nScale * pos.xyz, 3.553 + seed * 100.0 + 0.4 * time ) ),
      snoise( vec4( nScale * pos.xyz, 1.259 + seed * 200.0 + 0.4 * time ) )
    ) * 0.2;
    
    vel += -(pos) * length(pos) * 1.0;
