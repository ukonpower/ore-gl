...
void main() {
    ...
    float nScale = .3;
    vel.xyz += 20.0 * vec3(
      snoise( vec4( nScale * pos.xyz, 7.225 + t ) ),
      snoise( vec4( nScale * pos.xyz, 3.553 + t ) ),
      snoise( vec4( nScale * pos.xyz, 1.259 + t ) )
    ) * 0.1;
    
    // vec3 bp = pos - (point * uv.x);
    vel += (point) * max(0.0,(1.4 - distance(point,pos) * 0.3));
